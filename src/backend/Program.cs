...
builder
    .Services.AddIdentity<IdentityUser, IdentityRole>(options => // Usando IdentityUser e IdentityRole padrão
    {
...
var jwtSecret =
    jwtSection["Secret"] ?? throw new InvalidOperationException("JWT:Secret não configurado.");
var jwtIssuer =
    jwtSection["ValidIssuer"]
    ?? throw new InvalidOperationException("JWT:ValidIssuer não configurado.");
var jwtAudience =
    jwtSection["ValidAudience"]
    ?? throw new InvalidOperationException("JWT:ValidAudience não configurado.");
...
builder.Services.AddCors(options =>
{
    options.AddPolicy(
        ...
        {
            policyBuilder
                .WithOrigins(jwtAudience) // URL do seu app React (Vite default: http://localhost:5173)
                .AllowAnyHeader()
                .AllowAnyMethod();
        }
    );
